            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-2xl font-semibold">Event Ticket #{{ $eventTicket->id }}</h3>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <p class="font-medium">#{{ $eventTicket->id }}</p>
                                </div>
                                
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Booking Date</p>
                                    <p class="font-medium">{{ $eventTicket->created_at->format('M d, Y') }}</p>
                                </div>
                                
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Status</p>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                        {{ ucfirst($eventTicket->status) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <p class="font-medium">{{ $eventTicket->event->name }}</p>
                                </div>
                                
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Date</p>
                                    <p class="font-medium">{{ $eventTicket->event->date->format('M d, Y') }}</p>
                                </div>
                                
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Location</p>
                                    <p class="font-medium">{{ $eventTicket->event->location }}</p>
                                </div>
                                
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Tickets</p>
                                    <p class="font-medium">{{ $eventTicket->quantity }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex space-x-4">
                        <a href="{{ route('event-tickets.edit', $eventTicket) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Edit Ticket
                        </a>
                        
                        <form action="{{ route('event-tickets.destroy', $eventTicket) }}" method="POST">
                            @csrf
                            @method('DELETE')
